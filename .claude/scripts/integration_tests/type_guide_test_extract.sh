...
#   spawn <type_name>         - Get spawn format for a type
...
if [ $# -lt 2 ]; then
    ...
    echo "Operations: summary, discovered_count, type_info, mutation_paths, spawn, schema_info, validate_field"
    exit 1
fi
...
case "$OPERATION" in
    ...
    "spawn")
        if [ -z "$TYPE_NAME" ]; then
            echo "Error: type_name required for spawn operation"
            exit 1
        fi
        jq --arg type "$TYPE_NAME" '.type_guide[$type].spawn' "$FILE_PATH"
        ;;
    "schema_info")
        if [ -z "$TYPE_NAME" ]; then
    ...
    *)
        ...
        echo "Valid operations: summary, discovered_count, type_info, mutation_paths, spawn, schema_info, validate_field"
        exit 1
        ;;
esac
