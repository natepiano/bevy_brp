...
**Changes**:
1. Add `use serde::ser::SerializeMap;` import at top of file
2. Add `is_null_equivalent()` method to Example enum
3. Add SpawnInsertExample enum with custom Serialize implementation
4. Add stub Deserialize implementation for SpawnInsertExample
...
**Expected Outcome**: Code compiles successfully with new types available but unused.
...
**Change Type**: Atomic Group (Breaking - must be done together)
...
**Changes**:
1. In constants.rs: Rename 4 constants and update their text values
2. In api.rs: Add constant imports
3. In api.rs: Create extract_spawn_insert_example() function replacing extract_spawn_format()
...
**Expected Outcome**: New function available, old function still exists (will be removed in Step 3).
...
**Change Type**: Atomic Group (Breaking - all TypeGuide changes done together)

**Files to Modify**:
- `mcp/src/brp_tools/brp_type_guide/guide.rs`

**Changes**:
1. Change field from `spawn_format: Option<Value>` to `spawn_insert_example: Option<SpawnInsertExample>`
2. Add `#[serde(flatten)]` attribute to new field
3. Update build() method to call extract_spawn_insert_example()
4. Simplify generate_agent_guidance() signature and implementation
5. Delete extract_spawn_format_if_spawnable() helper method
6. Update error builders to use new field name
...
**Dependencies**: Requires Steps 1 and 2 completed

**Expected Outcome**: TypeGuide uses new structure, all spawn_format references removed.
...
**Change Type**: Export updates (Safe)

**Files to Modify**:
- `mcp/src/brp_tools/brp_type_guide/mutation_path_builder/mod.rs`

**Changes**:
1. Add `pub(super) use types::SpawnInsertExample;`
2. Change `pub(super) use api::extract_spawn_format;` to `pub(super) use api::extract_spawn_insert_example;`
...
**Dependencies**: Requires Steps 1-3 completed

**Expected Outcome**: New API surface exposed through module system.
...
2. Launch test app and verify TypeGuide serialization:
   ```bash
   # Launch a BRP-enabled example and check type guide output
   ```
...
**Expected Outcome**: ✅ All tests pass, serialization produces expected JSON structure.

---

## Implementation Details

### Step 1 Details: Add New Types and Helper Methods
...
### Step 2 Details: Rename Constants and Update Usage

**File 1**: `mcp/src/brp_tools/brp_type_guide/constants.rs`
...
**File 2**: `mcp/src/brp_tools/brp_type_guide/mutation_path_builder/api.rs`
...
```
(Note: These constants were renamed in the first part of this step)
...
### Step 3 Details: Update TypeGuide Structure
...
### Step 4 Details: Update Module Exports
