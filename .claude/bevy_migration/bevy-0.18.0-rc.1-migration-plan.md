...
## Update Cargo.toml Dependencies (FIRST STEP)

Before making any code changes, update the Bevy version in all Cargo.toml files:

### Workspace Cargo.toml (`/home/natemccoy/rust/bevy_brp/Cargo.toml`)
...
### extras/Cargo.toml (`/home/natemccoy/rust/bevy_brp/extras/Cargo.toml`)
...
**Note:** Run `cargo check` after updating Cargo.toml. The build will fail until the code changes below are completed.
...
1. Start with REQUIRED changes (must fix to compile with Bevy 0.18.0-rc.1)
...
6. Update CHANGELOGs for all 3 crates (see above)
