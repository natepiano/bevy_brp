...
fn collect_children_for_chain(
    ...
    child_paths
        ...
        .filter(|child| {
            target_chain
                .map(|chain| is_variant_chain_compatible(child, chain))
                .unwrap_or(true)
        })
    ...
    child_paths
        ...
        .flat_map(|child| {
            child
                ...
                .into_iter()
                .flat_map(|partials| {
                    partials.keys().filter_map(|child_chain| {
                        child_chain
                            .starts_with(our_chain)
                            .then(|| child_chain.clone())
                    })
                })
        })
        .collect()
