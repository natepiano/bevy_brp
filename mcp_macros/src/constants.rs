// result struct
pub(crate) const DEFAULT_DURATION_MS: u32 = 100;
pub(crate) const CHARS_QUEUED_FIELD: &str = "chars_queued";
pub(crate) const COMPONENTS_FIELD: &str = "components";
pub(crate) const DEBUG_ENABLED_FIELD: &str = "debug_enabled";
pub(crate) const DURATION_MS_FIELD: &str = "duration_ms";
pub(crate) const ENTITY_FIELD: &str = "entity";
pub(crate) const ERRORS_FIELD: &str = "errors";
pub(crate) const KEYS_SENT_FIELD: &str = "keys_sent";
pub(crate) const MESSAGE_FIELD: &str = "message";
pub(crate) const METHODS_FIELD: &str = "methods";
pub(crate) const RESULT_FIELD: &str = "result";
pub(crate) const SKIPPED_FIELD: &str = "skipped";
pub(crate) const STATUS_FIELD: &str = "status";
pub(crate) const TYPE_GUIDE_FIELD: &str = "type_guide";
pub(crate) const UNKNOWN_STATUS: &str = "unknown";
